  ...
  return (
    <div className='mt-12 flex gap-x-8 gap-y-16 justify-between flex-wrap '>
      ...
      >
        ...
        <button className='rounded-2xl ring-1 ring-lama text-lama py-2 px-4 w-max text-xs hover:bg-lama hover:text-white'>Add to Cart</button>
      </Link>
    ))}

    {searchParams?.cat || searchParams?.name ? 
    <Pagination currentPage={res.currentPage || 0} hasPrev={res.hasPrev()} hasNext={res.hasNext()}   /> : null}
    </div>
  )
}
